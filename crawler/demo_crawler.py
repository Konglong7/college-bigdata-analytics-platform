"""
demo_crawler.py - 全国高校大数据平台 · 答辩与现场演示专用采集引擎
设计目标:
  1. 自动化新建/重置独立的测试演示数据库 (univ_demo_db)，完全与生产库物理隔离
  2. 现场执行协议逆向与加盐签名计算 (HMAC-SHA1 + Base64 + MD5)，向评委老师直观自证真实采集能力
  3. 现场实时抓取全国代表高校最新名录与 2023-2024 真实高考录取分数线
  4. 现场清洗并批量持久化写入 univ_demo_db 测试库
  5. 现场从测试数据库中执行 SQL 查询并打印结构化表格，呈现完整的数据采集与入库闭环
  6. 全程耗时稳定控制在 15~20 秒，绝不冷场、绝不超时
"""
