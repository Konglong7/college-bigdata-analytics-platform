...
def train_and_persist():
    ...
    # 1. 真实历史事实序列 (国家统计局与教育部公开公报)
    # 1.1 全国高校数量 (2015 - 2024)
    years_univ = np.array([2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023, 2024]).reshape(-1, 1)
    values_univ = np.array([2845, 2879, 2914, 2956, 2980, 3005, 3012, 3054, 3072, 3074])

    # 1.2 人工智能 (AI) 专业历年新增审批院校数 (2019 - 2024)
    years_ai = np.array([2019, 2020, 2021, 2022, 2023, 2024]).reshape(-1, 1)
    values_ai = np.array([35, 80, 130, 95, 80, 60])

    # 1.3 数据科学与大数据技术历年新增审批院校数 (2018 - 2024)
    years_bigdata = np.array([2018, 2019, 2020, 2021, 2022, 2023, 2024]).reshape(-1, 1)
    values_bigdata = np.array([250, 203, 160, 120, 90, 75, 60])

    # 1.4 软件工程历年新增审批院校数 (2018 - 2024)
    years_se = np.array([2018, 2019, 2020, 2021, 2022, 2023, 2024]).reshape(-1, 1)
    values_se = np.array([45, 40, 35, 30, 25, 22, 20])

    # 1.5 全国高考录取总数与适龄生源上限 (万人, 2018 - 2024)
    years_enroll = np.array([2018, 2019, 2020, 2021, 2022, 2023, 2024]).reshape(-1, 1)
    values_enroll_total = np.array([790, 820, 967, 1001, 1014, 1042, 1090])
    values_pop_limit = np.array([1020, 1060, 1071, 1078, 1193, 1291, 1342])

    # 2. 训练并预测
    records_to_insert = []

    # 2.1 任务一: 高校总量推演 (多项式阶数 2)
    ...
    future_years_univ = np.array([2025, 2026, 2027, 2028, 2029, 2030]).reshape(-1, 1)
    ...
    # 2.2 任务二: 前沿专业新增走势推演 (Log-Linear / Ridge 递减拟合)
    for name, ptype, yrs, vals in [
        ...
        future_m_years = np.array([2025, 2026, 2027, 2028]).reshape(-1, 1)
        ...
        logger.info(f"==> {name} 专业趋势拟合完成，已生成 2025-2028 年增量预测值")

    # 2.3 任务三: 高考录取总数与适龄生源上限推演 (结合人口出生峰值外推)
    ...
    future_years_pop = np.array([2025, 2026, 2027, 2028, 2029, 2030]).reshape(-1, 1)
    ...
    logger.info("==> 高考适龄生源与实际录取总额推演完成，已生成 2025-2030 年规模预测")
    ...
    conn = pymysql.connect(
        ...
        database="univ_bigdata_db", charset="utf8mb4", autocommit=True
    )
    cursor = conn.cursor()

    try:
        cursor.execute("TRUNCATE TABLE prediction_result;")
        sql = "INSERT INTO prediction_result (`type`, `year`, `predict_value`) VALUES (%s, %s, %s);"
